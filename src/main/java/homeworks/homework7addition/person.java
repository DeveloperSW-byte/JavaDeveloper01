...
public class person {
    ...
    public void adddiscountProductToBasket(DiscountProduct discountProduct){
        if (this.money >= discountProduct.getPrice()){
            this.basket[index] = discountProduct;
            this.money -= discountProduct.getPrice();
            index++;
            System.out.println("Продукт " + discountProduct.getName() + " успешно добавлен в корзину.");
        } else {
            System.out.println(getName() + " не может позволить себе " + discountProduct.getName());
        }
        if (this.money == 0){
            System.out.println("Ничего не куплено");
    ...
    public person(String name, Integer money, Integer age){
        if (money < 0) {
